...
fn swap2_u32(x: u32, y: &mut u32) -> u32
{
  swapn_u32(0x55555555, 0xAAAAAAAA, 1, x, y)
}

fn swap4_u32(x: u32, y: &mut u32) -> u32
{
  swapn_u32(0x33333333, 0xCCCCCCCC, 2, x, y)
}

fn swap8_u32(x: u32, y: &mut u32) -> u32
{
  swapn_u32(0x0F0F0F0F, 0xF0F0F0F0, 4, x, y)
}

fn br_aes_ct_ortho(q: &mut [u32]) 
{
  q[0] = swap2_u32(q[0], &mut q[1]);
  q[2] = swap2_u32(q[2], &mut q[3]);
  q[4] = swap2_u32(q[4], &mut q[5]);
  q[6] = swap2_u32(q[6], &mut q[7]);

  q[0] = swap4_u32(q[0], &mut q[2]);
  q[1] = swap4_u32(q[1], &mut q[3]);
  q[4] = swap4_u32(q[4], &mut q[6]);
  q[5] = swap4_u32(q[5], &mut q[7]);

  q[0] = swap8_u32(q[0], &mut q[4]);
  q[1] = swap8_u32(q[1], &mut q[5]);
  q[2] = swap8_u32(q[2], &mut q[6]);
  q[3] =swap8_u32(q[3], &mut q[7]);
}
