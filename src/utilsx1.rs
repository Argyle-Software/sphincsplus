...
) 
{
    ...
    loop {
        ...
        loop {
            ...
            // let left: &mut[u8] = &stack[h * SPX_N..];
            current[..SPX_N].copy_from_slice(&stack[start..start + SPX_N]);
            let mut buf = [0u8; SPX_ADDR_BYTES + 2 * SPX_N];
            let mut tmp_current = current.clone();
            thash( &mut current[SPX_N..], &tmp_current, 2, &mut buf, ctx, *tree_addr);
          h += 1; 
          internal_idx >>= 1;
          internal_leaf >>= 1;
        }
...
) 
{
    ...
    // let mut stack = if tree_height == TreeHeight::SpxForsHeight {
    //   [0u8; SPX_FORS_HEIGHT*SPX_N]
    // } else {
    //   [0u8; SPX_TREE_HEIGHT*SPX_N]
    // };
    // match tree_height {
      // TreeHeight::SpxForsHeight => { stack = [0u8; SPX_FORS_HEIGHT*SPX_N]; }
      // TreeHeight::SpxTreeheight => { stack = [0u8; SPX_TREE_HEIGHT*SPX_N]; }
    // };
