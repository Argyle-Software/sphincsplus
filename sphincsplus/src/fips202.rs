...
**************************************************/
pub(crate) fn store64(x: &mut[u8], u: u64)
{
  for i in 0..8 {
    x[i] = (u >> 8 * i) as u8;
  }
...
**************************************************/
pub fn keccak_absorb(state: &mut KeccakState, r: usize, input: &[u8], mut inlen: usize) {
  let mut idx = 0;
 ...
 **************************************************/
pub fn keccak_inc_absorb(s_inc: &mut [u64], r: usize, m: &[u8], mut mlen: usize) {
  let mut idx = 0;
  /* Recall that s_inc[25] is the non-absorbed bytes xored into the state */
  while (mlen + s_inc[25] as usize >= r) {
    let mut i = 0;
    while i < r - s_inc[25] as usize {
      /* Take the i'th byte from message
      xor with the s_inc[25] + i'th byte of the state; little-endian */
      s_inc[(s_inc[25] as usize + i) >> 3] ^= (m[i] as u64) << (8 * ((s_inc[25] + i as u64) & 0x07));
      i += 1;
    }
    mlen -= (r - s_inc[25] as usize);
    idx += r - s_inc[25] as usize;
    s_inc[25] = 0;
    ...
    for i in 0..mlen {
    s_inc[(s_inc[25] as usize + i) >> 3] ^= (m[i] as u64) << (8 * ((s_inc[25] as usize + i) & 0x07));
    }
...
**************************************************/
pub fn keccak_finalize(s: &mut[u64; 25], pos: usize, r: usize, p: u8) {
 ...
 **************************************************/
 pub fn keccak_inc_finalize(s_inc: &mut[u64], r: usize, p: u8) {
    /* After keccak_inc_absorb, we are guaranteed that s_inc[25] < r,
       so we can always use one more byte for p in the current state. */
    s_inc[s_inc[25] as usize >> 3] ^= (p as u64) << (8 * (s_inc[25] & 0x07));
    s_inc[(r - 1) >> 3] ^= 128 << (8 * ((r - 1) & 0x07));
    s_inc[25] = 0;
}
...
* Returns new position pos in current block
**************************************************/
pub fn keccak_squeeze(out: &mut[u8], mut outlen: usize, s: &mut[u64; 25], mut pos: usize, r: usize) -> usize 
{
 ...
 *              - uint32_t r: rate in bytes (e.g., 168 for SHAKE128)
 **************************************************/
 pub fn keccak_inc_squeeze(h: &mut[u8], mut outlen: usize, s_inc: &mut[u64], r: usize) 
{
  let mut i = 0;
  while i < outlen && i < s_inc[25] as usize {
    h[i] = (s_inc[((r - s_inc[25] as usize + i) >> 3) as usize] >> (8 * ((r - s_inc[25] as  usize + i) & 0x07))) as u8;
    i += 1;
  }
  let mut idx = i;
  outlen -= i;
...
**************************************************/
pub(crate) fn shake128_squeeze(out: &mut[u8], outlen: usize, state: &mut KeccakState)
{
...
*              - keccak_state *s: pointer to input/output Keccak state
**************************************************/
pub(crate) fn shake128_squeezeblocks(output: &mut[u8], nblocks: usize, s: &mut KeccakState)
{
...
*              - size_t inlen: length of input in bytes
**************************************************/
pub(crate) fn shake256_absorb(state: &mut KeccakState, input: &[u8], inlen: usize)
{
...
* Arguments:   - keccak_state *state: pointer to Keccak state
**************************************************/
pub(crate) fn shake256_finalize(state: &mut KeccakState)
{
...
*              - keccak_state *s: pointer to input/output Keccak state
**************************************************/
pub(crate) fn shake256_squeeze(out: &mut[u8], outlen: usize, state: &mut KeccakState)
{
...
*              - keccak_state *s: pointer to input/output Keccak state
**************************************************/
pub(crate) fn shake256_squeezeblocks(out: &mut[u8], nblocks: usize, state: &mut KeccakState)
{
...
*              - size_t inlen: length of input in bytes
**************************************************/
pub(crate) fn shake128(output: &mut[u8], mut outlen: usize, input: &[u8], inlen: usize)
{
...
pub(crate) fn shake256(output: &mut[u8], mut outlen: usize, input: &[u8], inlen: usize)
{
  let mut state = KeccakState::default();
...
//              - const unsigned char *input: pointer to input
//              - unsigned long long inlen:   length of input in bytes
pub(crate) fn sha3_256(output: &mut [u8], input: &[u8], inlen: usize)
{
  let mut s =[0u64; 25];
...
//              - const unsigned char *input: pointer to input
//              - unsigned long long inlen:   length of input in bytes
pub(crate) fn sha3_512(output: &mut [u8], input: &[u8], inlen: usize) 
{
  let mut s =[0u64; 25];
